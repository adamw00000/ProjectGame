    ...
    public class GameMaster
    {
        ...
        public void MoveAgent(int agentId, MoveDirection moveDirection, string messageId)
        {
            ...
            try
            {
                ...
                response = new ActionMakeMoveResponse(agentId, timestamp, waitUntil, distance, messageId);
            }
            catch (PendingLeaderCommunicationException e)
            {
                (int timestamp, int waitUntil) = CalculateDelay(agentId);
                logger.Warn(e, $"Agent {agentId} couldn't move: ");
                response = new InvalidAction(agentId, CurrentTimestamp(), messageId);
            }
            catch (DelayException e)
            {
                logger.Warn(e, $"Agent {agentId} tried move during penalty: ");
                (int timestamp, int waitUntil) = CalculateDelay(agentId);
                response = new RequestTimePenaltyError(agentId, timestamp, waitUntil, messageId);
            }
            catch (InvalidMoveException e)
            {
                logger.Warn(e, $"Agent {agentId} couldn't move: ");
                response = new InvalidMoveDirectionError(agentId, CurrentTimestamp(), messageId);
            }
            connection.Send(response);
        }
        ...
        public void PickPiece(int agentId, string messageId)
        {
            ...
            try
            {
                ...
                response = new ActionPickPieceResponse(agentId, timestamp, waitUntil, messageId);
            }
            catch (PendingLeaderCommunicationException e)
            {
                (int timestamp, int waitUntil) = CalculateDelay(agentId);
        ...
        public void PutPiece(int agentId, string messageId)
        {
            ...
            try
            {
                ...
                response = new ActionPutPieceResponse(agentId, timestamp, waitUntil, result, messageId);
            }
            catch (PendingLeaderCommunicationException e)
            {
                (int timestamp, int waitUntil) = CalculateDelay(agentId);
        ...
        public void Discover(int agentId, string messageId)
        {
            ...
            try
            {
                ...
                response = new ActionDiscoverResponse(agentId, timestamp, waitUntil, discoveryResult, messageId);
            }
            catch (PendingLeaderCommunicationException e)
            {
                (int timestamp, int waitUntil) = CalculateDelay(agentId);
        ...
        public void CheckPiece(int agentId, string messageId)
        {
            ...
            try
            {
                ...
                response = new ActionCheckPieceResponse(agentId, timestamp, waitUntil, result, messageId);
            }
            catch (PendingLeaderCommunicationException e)
            {
                (int timestamp, int waitUntil) = CalculateDelay(agentId);
        ...
        public void DestroyPiece(int agentId, string messageId)
        {
            ...
            try
            {
                ...
                response = new ActionDestroyPieceResponse(agentId, timestamp, waitUntil, messageId); 
            }
            catch (PendingLeaderCommunicationException e)
            {
                (int timestamp, int waitUntil) = CalculateDelay(agentId);
        ...
        public void CommunicationRequestWithData(int requesterAgentId, int targetAgentId, object data, string messageId)
        {
            ...
            catch (PendingLeaderCommunicationException e)
            {
                ...
                Message response = new InvalidAction(requesterAgentId, CurrentTimestamp(), messageId);
                connection.Send(response);
            }
            catch (DelayException e)
            {
                (int timestamp, int waitUntil) = CalculateDelay(requesterAgentId);
                logger.Warn(e, $"Agent {requesterAgentId} couldn't request communication with {targetAgentId} and data {data.ToString()}");
                Message response = new RequestTimePenaltyError(requesterAgentId, timestamp, waitUntil, messageId);
                connection.Send(response);
            }
        }
        ...
        public void CommunicationAgreementWithData(int requesterAgentId, int targetAgentId, bool agreement, object targetData, string targetMessageId)
        {
            ...
            catch (PendingLeaderCommunicationException e)
            {
                ...
                Message response = new InvalidAction(targetAgentId, timestamp, targetMessageId);
                connection.Send(response);
                return;
            }
            ...
            if (targetData == null)
            {
                ...
                Message response = new InvalidAction(targetAgentId, CurrentTimestamp(), targetMessageId);
                connection.Send(response);
                return;
            }
            ...
            responseToSender = new ActionCommunicationResponseWithData(requesterAgentId, timestamp1, waitUntil1, targetAgentId, true, targetData, senderData.senderMessageId);
            responseToTarget = new ActionCommunicationResponseWithData(targetAgentId, timestamp2, waitUntil2, requesterAgentId, true, senderData.data, targetMessageId);
